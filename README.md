...
```
You are helping me set up Bimonth on this macOS machine. First read
`README.md` and `CLAUDE.md` for context, then:

1. Confirm Xcode 16+ is installed (`xcodebuild -version`). If not,
   stop and tell me to install it from the Mac App Store.
2. Install `xcodegen` if it's missing (`brew install xcodegen`).
3. Ask me for a reverse-DNS bundle prefix (e.g. `com.alice.bimonth`)
   and replace every occurrence of `com.example` in `project.yml`
   with it.
4. Run `xcodegen generate`.
5. Build the Debug `Bimonth` scheme for macOS with `xcodebuild`.
6. On a successful build, open the produced `Bimonth.app` and walk
...
If any step fails, stop and ask before proceeding. Do not commit,
push, or modify anything outside this repo.
```
